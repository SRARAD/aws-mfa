...
def main():
    ...
    parser.add_argument(
        "--region", help="Region to specify " "--region", required=False
    )
    parser.add_argument(
        "--force",
...
def validate(args, config):
    if not args.profile:
        if os.environ.get("AWS_PROFILE"):
            ...
            args.profile = "gdit-test"

    if not args.region:
        args.region = "us-gov-west-1"
    ...
    if args.assume_role:
        ...
        role_msg = ""
    logger.info(
        "Validating credentials for profile: %s %s" % (short_term_name, role_msg)
    )
...
def get_credentials(short_term_name, lt_key_id, lt_access_key, args, config):
    ...
    if args.assume_role:
        ...
        if args.role_session_name is None:
            log_error_and_exit(
                logger,
                "You must specify a role session name " "via --role-session-name",
            )
